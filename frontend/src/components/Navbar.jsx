...
const Navbar = ({ links = [], cta }) => {
  ...
  const defaultLinks = [
    ...
    { to: 'https://www.techwatt.ai/shop/', label: 'Shop', external: true },
  ];

  const navLinks = links.length ? links : defaultLinks;

  const defaultCta = (
    <Link to="/study" className="btn btn-primary btn-sm ml-2">
      Explore Courses
    </Link>
  );

  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-tw-border bg-tw-surface/95 backdrop-blur-xl shadow-sm">
      <div className="page-container">
        <div className="flex h-16 items-center justify-between">
          <Link to="/" className="flex items-center gap-2.5 shrink-0" onClick={() => setOpen(false)}>
            <img src={logo} alt="TechWatt" className="h-8 w-auto" />
            <span className="text-base sm:text-lg font-bold gradient-text truncate">TechWatt</span>
          </Link>
          ...
          <button
            className="md:hidden touch-target p-2 rounded-lg text-tw-muted hover:text-tw-text hover:bg-tw-surface-2 transition-colors"
      ...
      {open && (
        <div className="md:hidden border-t border-tw-border bg-tw-surface animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="page-container py-4 flex flex-col gap-1">
            ...
            <Link
              to="/study"
              className="btn btn-primary btn-block mt-2 mx-4"
              onClick={() => setOpen(false)}
            >
              Explore Courses
            </Link>
